...
public class Main {
	...
	public static void solve() {
		...
		int[][] map = new int[N][M];
		HashSet<Integer> set = new HashSet<>(); // 꽃의 위치
		...
		while(!que.isEmpty()) {
			...
			for(int i=0;i<4;i++) {
				...
				if(!v[nx][ny] && garden[nx][ny] != 0) {
					v[nx][ny] = true;
					map[nx][ny] = curr.seq+1;
					if(!curr.color)
						map[nx][ny] *= -1;
					que.add(new CultureMedium(nx, ny, curr.color,curr.seq+1));	
				}else if(v[nx][ny] && Math.abs(map[nx][ny]) == curr.seq+1 
						&& !set.contains(nx*M+ny)) {
					if(curr.color && map[nx][ny]<0) { // true면 빨강
						set.add(nx*M+ny);
					}else if(!curr.color && map[nx][ny] >0)
						set.add(nx*M+ny);
				}
			}
		}
